...
function Register() {
  ...
  const validateForm = () => {
    ...
    // Nâng cấp: Dùng chuẩn validate mạnh
  ...
  const handleKeyDownOtp = (e, index) => {
    // Tự động lùi về ô trước khi xóa
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        {step === 1 && (
          <form onSubmit={handleRequestOtp}>
            <div className="form-group">
              ...
              <input type="text" id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Nhập họ và tên" disabled={loading} />
              {errors.fullName && <span className="error">{errors.fullName}</span>}
            </div>

            <div className="form-group">
              <label htmlFor="email">Email</label>
              <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} placeholder="Nhập email" disabled={loading} />
              {errors.email && <span className="error">{errors.email}</span>}
            </div>

            <div className="form-group">
              <label htmlFor="phone">Số điện thoại</label>
              <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleChange} placeholder="Nhập số điện thoại" disabled={loading} />
              {errors.phone && <span className="error">{errors.phone}</span>}
            </div>

            <div className="form-group">
              <label htmlFor="address">Địa chỉ</label>
              <input type="text" id="address" name="address" value={formData.address} onChange={handleChange} placeholder="Nhập địa chỉ" disabled={loading} />
            ...
            <div className="password-requirements" style={{ marginBottom: '24px' }}>
              ...
              <ul>
                ...
                <li className={/[^A-Za-z0-9]/.test(formData.password) ? 'requirement-met' : ''}>
                  <i className={`fas ${/[^A-Za-z0-9]/.test(formData.password) ? 'fa-check' : 'fa-times'}`}></i>
                  Chứa ký tự đặc biệt (!@#$%^&*)
                </li>
              </ul>
            </div>
        ...
        {/* BƯỚC 2: Form nhập OTP 6 ô */}
        {step === 2 && (
          <form onSubmit={handleVerifyOtp}>
            <p className="auth-description" style={{ marginBottom: '20px' }}>
              Chúng tôi đã gửi một mã xác thực 6 số đến email <strong>{formData.email}</strong>. Mã có hiệu lực trong vòng 5 phút.
            </p>
            ...
            <div className="form-group">
              <label style={{ textAlign: 'center', marginBottom: '15px' }}>Nhập mã OTP</label>
              ...
              {errors.otpCode && <span className="error" style={{ textAlign: 'center' }}>{errors.otpCode}</span>}
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '20px' }}>
              <button 
                type="button" 
                onClick={() => setStep(1)}
                style={{ background: 'none', border: 'none', color: '#1a1a1a', textDecoration: 'underline', cursor: 'pointer', fontWeight: '500' }}
                disabled={loading}
              >
