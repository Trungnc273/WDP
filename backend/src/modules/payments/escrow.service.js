...
async function releaseFunds(orderId, reason = 'Người mua đã xác nhận nhận hàng', isAuto = false) {
  ...
  try {
    ...
    // If auto-released (via cron), we must also update the Order and Product
    // since there's no buyer action triggering confirmReceipt.
    if (isAuto) {
      ...
      try {
        await notificationService.createNotification(order.sellerId, {
          ...
          message: `Hệ thống đã tự động hoàn tất đơn hàng sau 5 ngày. Tiền đã được chuyển vào ví của bạn.`
        });
      } catch (e) {
